...
4. Anchors every sample to a **sleep-aware monotonic clock** — `SystemClock.elapsedRealtime()` on Android and `mach_continuous_time()` on iOS — so the corrected time is computed as `ntp + (now − monotonic)`.
...
**Why a monotonic clock?** The device wall clock (`Date.now()`) can be changed by the user or by automatic time corrections. Because `getTime()` is anchored to the monotonic clock — which cannot be changed manually — the returned time stays reliable even if the user changes the device date/time _after_ the first successful sync.

**Why a native module?** Before 2.0.0 the package used `performance.now()` as its monotonic clock. That clock is _sleep-blind_ on every platform: on Android's Hermes runtime it is backed by `SystemClock.uptimeMillis()` and on iOS by `mach_absolute_time()`, and **neither counts time spent in deep sleep**. A phone synced at 21:00, closed overnight, and reopened offline at 08:10 would project the time as ~01:27 (≈7h behind).

Since 2.0.0 the package ships a small native module (`RNNtpMonotonicClock`) that reads `SystemClock.elapsedRealtime()` on Android and `mach_continuous_time()` on iOS — both include deep sleep, are immune to wall-clock manipulation, and reset on reboot (preserving the existing boot-filter semantics). The module exposes a `clock` constant, and the JS layer uses it to know exactly which clock it is projecting on.

**Fail-safe fallback.** Consumers must rebuild their native code (and re-run `pod install` on iOS) for the module to be linked. Until then — and on platforms without the module (e.g. web) — the package falls back to `performance.now()` and **reports `MONOTONIC_CLOCK = 'uptime'`**: it never claims a sleep-aware clock it doesn't have. Because `importDeltas` accepts only deltas whose `clock` matches the clock in use, `'elapsed'` deltas persisted on a real device are discarded while the fallback is active, instead of being silently projected onto the wrong clock.

> **Breaking change (2.0.0):** persisted delta format. Deltas stored by versions < 2.0.0 used `performance.now()` (uptime-based) and have no `clock` field. Because uptime ≤ elapsed-realtime always, they would pass the boot filter and silently project the entire sleep time since boot. `importDeltas` now discards any delta whose `clock` field does not match the current clock (deltas without the field are treated as `'uptime'` and discarded). New deltas carry `clock: 'elapsed'` when the native module is available and `clock: 'uptime'` while the fallback is in use.
...
Each persisted sample must include `ntp` (the NTP time at sync), `monotonic` (the monotonic clock value at sync) and `clock` (the clock used to measure it). The monotonic clock is boot-based on Android (`SystemClock.elapsedRealtime`) and iOS (`mach_continuous_time()`), so raw values are valid across process restarts within the same boot — **no re-anchoring via `Date.now()` is needed**, which avoids reintroducing clock-manipulation vulnerabilities.
...
> **Persist safely:** only deltas measured on the sleep-aware clock are usable across app restarts. Persist them while `MONOTONIC_CLOCK === 'elapsed'` (native module linked). Deltas captured under the `'uptime'` fallback are discarded on import anyway, so storing them is harmless but useless.
...
Tests use Jest with a full mock of `react-native-udp`, covering NTP packet parsing, round-trip compensation, server validation, server rotation, monotonic anchor immunity to device clock changes, deep-sleep handling (sleep-aware monotonic clock), the `'uptime'` fallback when the native module is missing, legacy-delta rejection on import, offline reliability, secure monotonic-based delta import, maxSkewMs protection against rogue servers, and all public API methods (93 tests).

---

## Types

The package also exports `monotonicNow(): number` (current monotonic clock value in ms — sleep-aware on device, `performance.now()` fallback otherwise) and `MONOTONIC_CLOCK` (`'elapsed'` when the native module is linked, `'uptime'` otherwise), so consumers can persist/import deltas without deep imports:

```typescript
import { monotonicNow, MONOTONIC_CLOCK } from '@ruanitto/react-native-ntp-sync';
```
...
// 'uptime'  = SystemClock.uptimeMillis / performance.now() / mach_absolute_time —
//             pauses during deep sleep (pre-2.0.0, and the native-less fallback)
// 'elapsed' = SystemClock.elapsedRealtime / mach_continuous_time —
//             includes deep sleep (2.0.0+ with the native module linked)
...
type Delta = {
  ...
  clock: MonotonicClock; // matches the active source — 'elapsed' on device, 'uptime' fallback
};
...
type DeltaImport = {
  ...
  clock?: MonotonicClock; // optional — deltas without it are treated as 'uptime' and discarded on import
};
